 ...
 * numbers and cannot: platform/hsf/hsf_load_native.c builds a FRESH,
 ...
 * platform/os/arena.c's reservation. Scaling the capacities without scaling
 ...
 * Spelled with the same `*2`/`*8` factors platform/os/malloc_direct.c used
...
/* The reservation platform/os/arena.c made before this file existed. */
 ...
 * so it takes that entry). Named because platform/host/coro_arena.c has to
 ...
 * the 4 GB the whole arena is pinned under (platform/os/arena.c). */
...
/* ---------------------------------------------------------------------
 * The stateful half (platform/os/heap_scale.c).
 * ------------------------------------------------------------------ */
 ...
 * platform/os/arena.c actually reserved, and therefore the size
