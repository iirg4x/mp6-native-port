 ...
 *
 * ONE C SEAM. Nothing outside platform/gx/console/ and the four call sites
 ...
 * platform/gx/framescope.c's mp6_fs_frame_end() and
 * platform/gx/ui/launcher_core.cpp's mp6_launcher_frame_overlay() already
 * established.
 *
 * THREE UNITS BEHIND IT
 *   platform/gx/console/console_core.c   -- command registry, output ring,
 *       lever (cvar) table, tokenizer, history, open/available state. Plain
 *       C (stdio/string/stdarg only) in the style of platform/os/
 ...
 *   platform/gx/console/console_stats.c  -- the sampler and every stat
 *       panel's TEXT. Also in both builds, split internally by #ifdef
 *       MP6_HEADLESS_BUILD exactly like platform/gx/frame_dump.c.
 *   platform/gx/ui/console.cpp           -- the RmlUi view of the BAR (the
 ...
 *   platform/gx/ui/overlay.cpp           -- the RmlUi view of the STAT
...
/* THE input-capture predicate. platform/gx/aurora_bridge.c's
 ...
 * the bar. platform/gx/ui/overlay.cpp draws them, because that is the
 ...
 * platform/gx/ui/console.cpp turns each into RmlUi flex boxes with
 ...
 * read inside patches/decomp/ TUs, not in platform/, so a runtime setter
...
/* ---------------------------------------------------------------------
 * Sampler hooks (platform/gx/console/console_stats.c). Every one of these
 * is the single extern line a hot file receives.
 * ------------------------------------------------------------------- */

/* Called from platform/gx/aurora_bridge.c's mp6_present_counters_add() --
...
/* One HuPrc process slice, from platform/os/process_native.c's
 ...
 * rather than a call -- the shape platform/gx/framescope.c's mp6_fs_active()
 * and platform/gx/frame_dump.c's s_fdEnabled test already use. Owned by
...
/* ---------------------------------------------------------------------
 * Savestate (platform/os/savestate.c, beside mp6_fi_savestate_reset()).
