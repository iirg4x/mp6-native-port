/* MP6 native port -- developer console CORE. See shim/include/mp6_console.h
 * for the whole contract; this file is the mechanism.
 *
 * Deliberately dependency-light -- <stdio.h>/<stdarg.h>/<string.h> plus the
 * header-only strict parser -- exactly like platform/os/mp6_events.c, and in
 ...
 *
 * It owns every piece of console STATE. platform/gx/ui/console.cpp is a pure
 ...
 * the same failure class platform/gx/frame_dump.c is carved out for.
...
static void console_register_builtins(void)
{
    ...
    mp6_console_register("clear", "clear -- empty the output ring", cmd_clear);
 ...
 * is defined next to them in platform/gx/console/console_stats.c. Everything
