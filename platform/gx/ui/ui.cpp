...
/* [MP6] dropped: #include "RmlUi/Debugger/Debugger.h" */
...
void handle_event(const SDL_Event &event) noexcept
{
    ...
    if (event.type == SDL_EVENT_GAMEPAD_ADDED) {
        ...
        if (SDL_GamepadConnected(gamepad)) {
            { /* [MP6] always on -- our settings carry no controller-toast toggle */
...
{
    /* [MP6] resolved res/ base (cwd res/ if present, else exe-relative --
     * launcher_core.cpp) instead of their bare cwd-relative "res". */
    return std::filesystem::path(resource_base()) / filename;
}
