"""Verify matched W01 draw geometry and visualize the actual framebuffer crops.

Both runs must use Strong AO and the same quick state. Only the QA switch
--no-log-grounding differs; screenshots are decoded test output, not retouched.
"""
import json
...
def compare(before, after):
    ...
    for a,b in zip(old,new):
        ...
        if not changed.any():
            ...
            continue
        assert a['model']==31 and a['name'] in ('obj34','kirikabu','obj37'), a['name']
        assert np.max(np.abs(av[:,[0,2]]-bv[:,[0,2]])) < .002
        assert np.all(bv[changed,1] < av[changed,1])
        gaps=[]
        for p,q in zip(av[changed],bv[changed]):
            heights = [h for t in floor if (h:=height(t,*p[[0,2]])) is not None and h<=p[1]+.5 and p[1]-h<32]
            assert heights
            support = max(heights)
            gaps.append((p[1]-support,q[1]-support))
        gaps = np.array(gaps)
        assert np.all((gaps[:,0]>.5) & (gaps[:,0]<20))
        assert np.max(np.abs(gaps[:,1]+.1)) < .003
        report['bases'][a['name']] = dict(vertices=int(changed.sum()),
            before_gap_range=[float(gaps[:,0].min()),float(gaps[:,0].max())],
            after_gap_range=[float(gaps[:,1].min()),float(gaps[:,1].max())],
            unchanged_upper_vertices=int((~changed).sum()))
    assert set(report['bases']) == {'obj34','kirikabu','obj37'}
    (after/'log-grounding-metrics.json').write_text(json.dumps(report,indent=2)+'\n')
    ...
    # Identical native-resolution sample windows around the two visible bases.
    rectangles = [(735,275,1115,445),(0,420,450,590),(1190,305,1450,475)]
    canvas=Image.new('RGB',(920,660),(24,27,33))
    draw=ImageDraw.Draw(canvas)
    font=ImageFont.truetype('C:/Windows/Fonts/segoeui.ttf',20)
    draw.text((10,6),'Before — GTAO Strong',font=font,fill='white')
    draw.text((470,6),'After — GTAO Strong',font=font,fill='white')
    for row,box in enumerate(rectangles):
        x0,y0,x1,y1=box
        for col,(_,pixels) in enumerate(captures):
            canvas.paste(Image.fromarray(pixels[y0:y1,x0:x1]),(10+col*460,40+row*205))
    canvas.save(after/'log-grounding-before-after.png')
