# PARTYBOARD PROVENANCE NOTICE

*This notice is a hard requirement and must travel with the code it
describes.*

## What happened

At **explicit, repeated direction from this project's maintainer** (quoted
verbatim: "I WANT THE IMPLEMENTATION RIPPED FROM THE REPO … RIP IT, STITCH
IT, ADAPT IT TO MP6"), the launcher/settings UI implementation of the
**partyboard** project (the Mario Party 4 PC port) was **copied directly
into this repository** and adapted to MP6. The project's prior clean-room
rule (partyboard read as a spec only, all code re-expressed independently)
was **revoked by the maintainer for this component**. An earlier,
from-scratch ImGui reimplementation of this same UI (previously at
`platform/gx/launcher_menu.cpp`) was deleted in the same change.
...
| Destination | Originally copied into this project's private local repository. Files were copied at the maintainer's explicit direction; this notice documents that decision and its scope. **Do not publish or redistribute this repository (or these files) without resolving the upstream licensing situation first.** |
| Authorship credit | The UI framework files carry `// Credits: TwilitRealm` upstream; partyboard's own README credits the GC/Wii decompilation community, the Aurora developers, the Dusk developers, all partyboard contributors, "ImWhoreHay for the font", and "justcamtro for designing the assets". All such credit lines are preserved in the copies -- see this repository's own README for the carried-forward Credits section. |
...
licensed, RmlUi is MIT licensed, and this port's Aurora pin already
contained the module. Only the partyboard-authored files below are
unlicensed material.
...
does not have), `portmain.cpp` (read as wiring reference only).

OURS (not ripped, listed for completeness): `platform/gx/ui/
launcher_core.cpp` + `launcher_state.hpp` — the pre-existing MP6 glue
...
override for `.watermark` — in a later change; see this file's closing
section). No other rule was altered.

### Fonts: `res/<name>` → `res/fonts/<name>` (copied VERBATIM, per the maintainer's blanket order)
...
| `N64Party-Monochromatic.otf` | "N64 Party" | **fan font, no license text**; partyboard's README credits ImWhoreHay "for the font" (gifted to that project). Copied at the maintainer's explicit direction. |
| `FOT-NewRodin Pro DB.otf` | FOT-NewRodin Pro | **COMMERCIAL (Fontworks). No redistribution rights.** This is the single highest licensing risk in this repository. It was evaluated for removal before this snapshot was taken: `res/rml/*.rcss` selects `"FOT-NewRodin Pro"` as the primary body font for nearly all launcher/settings/tuner/popup text, RmlUi has no fallback face configured for that family, and a build with this font unloaded logs `aurora::rmlui` "No font face defined" warnings for those elements and renders no visible text there at all (confirmed by an actual build+run). It is kept, as shipped, for correct rendering — resolving this (swapping the stylesheets to a redistributable face and dropping the font) is the top item to clear before ever publishing this repository. |
...
1. Every ripped file keeps its upstream `// Credits: TwilitRealm` line and
   gains a header naming the source repo+commit, the maintainer's
   directive, and the no-license status.
...
## Android launcher addendum (2026-07-17): android build read as reference, nothing new ripped

Enabling the already-ripped RmlUi launcher on the Android build added one
new, unripped file: `platform/gx/ui/content_setup.cpp` / `.hpp` (the
first-run "Game Content Setup" dialog). It is **our own code**, built on
classes and stylesheet rules already covered by this notice
...
`external_refs/repos/partyboard`'s top-level `CMakeLists.txt` was read **as
a build-recipe reference only, not copied**, the same treatment this
notice already gives `portmain.cpp` (read as wiring reference only): it
confirmed `AURORA_ENABLE_RMLUI` is set unconditionally (not Android-gated)
in a project that ships the same ripped UI framework on Android in
production, and that their `ANDROID` branch builds a single `SHARED`
library named `main` for `SDLActivity` to `dlsym` — a shape this port's own
`libmain.so` bootstrap already used independently, not something copied
from this read. No CMake text, build logic, or file was copied from it;
`tools/build.py` remains the sole build brain for this port, unchanged in
kind by this observation.

## Mobile aspect-ratio addendum (2026-07-17): one new rule in our own additive block

A later mobile-aspect-ratio fix (launcher layout on phone/tablet aspect
ratios) added exactly one new CSS rule, entirely inside the already-
declared `[MP6] ADDITIVE BLOCK` at the end of `res/rml/prelaunch.rcss` — a
`@media (max-height: 640dp)` override that hides `.watermark` (our own
...
actual root cause was in our own C glue (`platform/gx/aurora_bridge.c`,
`platform/gx/ui/launcher_core.cpp`, `platform/main_native.c`): the RmlUi
launcher's presentation dimensions were being computed from the same
`AURORA_VIEWPORT_FIT`-fitted framebuffer size the GAME's 4:3 GX viewport
uses, so the launcher was quietly composed for a letterboxed 4:3
sub-rectangle instead of the real window/display surface — including how
partyboard's own `portmain.cpp` calls the same `AuroraSetViewportPolicy`
API at the same point in boot but defaults its equivalent config key
