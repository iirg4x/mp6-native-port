 ...
 * behind the src/ shim layer instead (arena, REL loader bridge, HuPrc
 ...
 *     src/null/shims_manual.c's own VIInit/VIWaitForRetrace/GXInit
 ...
 *     is dynamic); src/android/mp6launcher.c android_dlopen_ext()s
...
int main(int argc, char **argv)
{
     ...
     * screen. The engine is shared now (src/os/input_script.c), so the
 ...
 * six-function seam is src/gx/ui/launcher_core.cpp + the RmlUi
...
int main(int argc, char **argv) /* expands to aurora_main via aurora/main.h */
{
     ...
     * scripting for tests, see src/os/input_script.c's own header)
    ...
    /* Android's MP6_*= argument import has already completed before this
     * shared entry point. Later settings publications refresh the cache. */
    mp6_enh_cache_environment();
     ...
     * (disabled) this way -- our OWN game arena (src/os/arena.c, a
    ...
    /* Leave legacy cached shader configurations behind without deleting them.
     * A stale Last Five Turns configuration can crash startup precompilation.
     * Only graphics caches change location; saves/settings keep their paths. */
    char *gpuCachePath = mp6_bridge_gpu_cache_path(config.appName, getenv("MP6_GPU_CACHE_PATH"));
    if (gpuCachePath == NULL) {
        fprintf(stderr, "[BOOT] failed to prepare graphics cache\n");
        return 2;
    }
    config.cachePath = gpuCachePath;
    ...
    free(gpuCachePath); /* Aurora duplicates config.cachePath during initialization. */
     ...
     * Must run before the first game frame; see src/gx/aurora_bridge.c's
     ...
     * it exists -- see mp6_boot.h and src/gx/aurora_bridge.c. AFTER the
     ...
     * (include/mp6_enhancements.h), which is the single front door for
     ...
     * hardware) -- src/gx/aurora_bridge.c's VIWaitForRetrace is what
    ...
    fflush(stdout);
    { extern void mp6_launcher_ui_teardown(void); mp6_launcher_ui_teardown(); }
    aurora_shutdown();
    return 0;
}
