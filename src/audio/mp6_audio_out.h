 ...
 * (src/null/shims_manual.c) already used throughout this port:
 ...
 *   - default (aurora): src/audio/audio_out_sdl.c (tools/build.py's
 *     PLATFORM_AURORA_ONLY, compiled with Aurora's own flags/include path
 *     for <SDL3/SDL.h>) provides the real SDL3 device/stream.
 *
 * Called from msm_bridge.c's msmSysInit rather than src/main_native.c
 ...
 * reachable from here without touching src/os/*.c or
 * src/gx/aurora_bridge.c. Declared for completeness/future use;
