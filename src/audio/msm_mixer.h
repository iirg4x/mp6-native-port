 ...
 *   - default (aurora): src/audio/audio_out_sdl.c's SDL3 audio
 ...
 * Serialized with control-plane calls by the mixer mutex. Completed PCM is
 * retired in-place; the game thread reclaims it outside the mixer mutex. */
void mp6_msm_render(int16_t *outInterleaved, uint32_t frames);
/* Game/control thread only. No-op unless the callback retired a sample. */
