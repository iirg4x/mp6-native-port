...
/* Registry pointers, counts and contents must travel together in quick saves.
 * libc realloc put the contents outside the captured arena, leaving a restored
 * pointer into another process (or another timeline). This non-pointer tag is
 * private to the persistent registry, never an overlay/model lifetime tag. */
#define MP6_ANIM_REGISTRY_TAG UINT32_C(0x4d50414e)
...
static MP6AnimRecord *mp6_anim_record_slot(void)
{
    size_t i;
    MP6AnimRecord *grown;
    MP6AnimRecord *previous = s_animRecords;
    ...
    if (newCapacity <= oldCapacity
            || newCapacity > (size_t)INT32_MAX / sizeof(*s_animRecords)) {
        mp6_anim_fatal("cache capacity overflow");
    }
    grown = (MP6AnimRecord *)HuMemDirectMallocNum(HEAP_HEAP,
        (s32)(newCapacity * sizeof(*s_animRecords)), MP6_ANIM_REGISTRY_TAG);
    if (grown == NULL) {
    ...
    if (oldCapacity != 0) memcpy(grown, previous, oldCapacity * sizeof(*grown));
    memset(&grown[oldCapacity], 0,
           (newCapacity - oldCapacity) * sizeof(*s_animRecords));
    s_animRecords = grown;
    s_animRecordCapacity = newCapacity;
    if (previous != NULL) HuMemDirectFree(previous);
