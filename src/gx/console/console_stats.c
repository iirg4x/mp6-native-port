 ...
 *    src/gx/frame_interp.c owns a PERMANENT decline census and cost
 ...
 * shape src/gx/framescope.c's mp6_fs_active() and src/gx/
                                 ...
                                 long long slackNs)
{
    ...
    aurora_gpu_stats_set_enabled(mp6_console_overlay(MP6_CONSOLE_PANEL_GPU) ||
                                 mp6_console_overlay(MP6_CONSOLE_PANEL_UNIT));
#endif
 ...
 * per-stage breakdown maps onto PER-PROCESS execution time. src/os/
 ...
 * include/mp6_console.h beside mp6_console_panel_text(). Everything here
 ...
 *    include/mp6_diag_probe.h, which links in both.
 * ======================================================================= */
...
/* GPU pass timestamps are supplied asynchronously by the renderer. */
static void cs_panel_gpu(void)
{
    cs_title("GPU [STATGROUP_GPU]");
    cs_cols("Pass\tAverage ms\tMax ms");
#ifdef AURORA_GPU_STAT_ROWS
    AuroraGpuStats gpu;
    aurora_gpu_stats_read(&gpu);
    if (!gpu.status) { cs_note("GPU timing unavailable on this device."); return; }
    if (!gpu.sampleCount) {
        cs_note(gpu.status == 3 ? "GPU timing readback failed." : "Collecting samples...");
        return;
    }
    cs_group("Last %u frames", gpu.sampleCount);
    cs_row_begin('.',0);
    cs_cell("[GPU span]"); cs_cell("%.3f",gpu.spanAverageMs); cs_cell("%.3f",gpu.spanMaxMs);
    cs_row_end();
    for (unsigned i=0;i<gpu.rowCount;++i) {
        cs_row_begin('.',0);
        cs_cell("%s",gpu.rows[i].name);
        cs_cell("%.3f",gpu.rows[i].averageMs); cs_cell("%.3f",gpu.rows[i].maxMs);
        cs_row_end();
    }
    cs_row_begin('x',0);
    cs_cell("[Between passes]"); cs_cell("%.3f",gpu.betweenAverageMs); cs_cell("%.3f",gpu.betweenMaxMs);
    cs_row_end();
    if (gpu.droppedFrames) cs_note("Dropped samples: %u",gpu.droppedFrames);
#else
    cs_note("GPU timing unavailable in this build.");
#endif
}
 ...
 * (src/os/process_native.c) and named by the process's entry symbol.
 *
...
static void cs_panel_unavailable(const char *name)
{
    ...
    cs_note("Unavailable in headless mode.");
}
 ...
 * seam src/gx/frame_dump.c uses for aurora-patches/0025, because this TU
 ...
 * implementation (src/gx/aurora_bridge.c, PLATFORM_AURORA_ONLY) does not
 * link. */
 ...
 * its own SHAPE -- src/gx/ui/overlay.cpp gives it the corner the FPS
...
static void cs_panel_unit(void)
{
...
#ifdef AURORA_GPU_STAT_ROWS
    {
        AuroraGpuStats gpu;
        aurora_gpu_stats_read(&gpu);
        if (gpu.sampleCount) cs_hud_ms("GPU span", gpu.spanAverageMs, budgetMs);
        else cs_hud_text("GPU", gpu.status ? "..." : "unavailable");
    }
#else
    cs_hud_text("GPU", "unavailable");
#endif
...
void mp6_console_savestate_reset(void)
{
     ...
     * src/gx/frame_interp.c drops its retained streams at this same
