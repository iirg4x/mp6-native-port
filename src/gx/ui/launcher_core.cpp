 ...
 * Prelaunch) and pumps their frame loop; src/main_native.c consumes
...
void mp6_audio_set_master_gain(float gain);                                 /* src/audio/audio_out_sdl.c */
#ifdef __ANDROID__
...
/* The Enhancements seam (include/mp6_enhancements.h) names the same six
...
static Mp6EnhValues mp6_launcher_enh_values_of(const Mp6LauncherConfig &c)
{
    ...
    v.ambientOcclusion = c.ambientOcclusion;
    return v;
}
...
static void mp6_launcher_enh_store(const Mp6EnhValues *v)
{
    ...
    g_cfg.ambientOcclusion = v->ambientOcclusion;
}
...
static void mp6_launcher_defaults(void)
{
    ...
    g_cfg.touch = mp6_touch_defaults();
...
enum Mp6ConfigKeyBit : uint32_t {
    ...
    MP6_CFG_TOUCH_ENABLED = 1u << 25,
    MP6_CFG_TOUCH_SIZE = 1u << 26,
    MP6_CFG_TOUCH_OPACITY = 1u << 27,
    MP6_CFG_TOUCH_FLOATING = 1u << 28,
    MP6_CFG_TOUCH_LAYOUT = 1u << 29,
    MP6_CFG_ENH_AMBIENT_OCCLUSION = 1u << 30,
};

static uint32_t js_known_key_bit(const char *key)
{
    if (strcmp(key, "enhancements.ambient_occlusion") == 0) return MP6_CFG_ENH_AMBIENT_OCCLUSION;
    if (strcmp(key, "touch.enabled") == 0) return MP6_CFG_TOUCH_ENABLED;
    if (strcmp(key, "touch.size") == 0) return MP6_CFG_TOUCH_SIZE;
    if (strcmp(key, "touch.opacity") == 0) return MP6_CFG_TOUCH_OPACITY;
    if (strcmp(key, "touch.floating") == 0) return MP6_CFG_TOUCH_FLOATING;
    if (strcmp(key, "touch.layout") == 0) return MP6_CFG_TOUCH_LAYOUT;
...
static bool js_known_string_key(uint32_t bit)
{
    return bit == MP6_CFG_WINDOW_MODE || bit == MP6_CFG_BACKEND ||
           bit == MP6_CFG_CONTENT_ROOT || bit == MP6_CFG_ENH_PRESET ||
            bit == MP6_CFG_DISPLAY || bit == MP6_CFG_TOUCH_LAYOUT;
}
...
static int mp6_launcher_parse_config(const char *text)
{
    ...
    while (*p != '}') {
        ...
        if (kind == VALUE_STRING) {
            if (bit == MP6_CFG_TOUCH_LAYOUT) {
                mp6_touch_layout_read(sval, parsed.touch);
            } else if (bit == MP6_CFG_WINDOW_MODE) {
            ...
            else if (bit == MP6_CFG_TOUCH_ENABLED) parsed.touch.enabled = boolValue;
            else if (bit == MP6_CFG_TOUCH_FLOATING) parsed.touch.floating = boolValue;
            ...
            if (bit == MP6_CFG_TOUCH_SIZE) {
                parsed.touch.size = (v >= 75 && v <= 150 && v == std::floor(v)) ? (int)v : 100;
            } else if (bit == MP6_CFG_TOUCH_OPACITY) {
                parsed.touch.opacity = (v >= 20 && v <= 100 && v == std::floor(v)) ? (int)v : 55;
            } else if (bit == MP6_CFG_WINDOW_SCALE) {
            ...
            else if (bit == MP6_CFG_ENH_AMBIENT_OCCLUSION) enh.ambientOcclusion = js_enh_int(v);
     ...
     * names them. */
    {
        ...
        if ((seen & MP6_CFG_ENH_AMBIENT_OCCLUSION) != 0) resolved.ambientOcclusion = enh.ambientOcclusion;
        ...
        parsed.ambientOcclusion = resolved.ambientOcclusion;
    }
...
static void mp6_launcher_config_save(void)
{
    ...
    char touchLayout[256];
    mp6_touch_layout_write(g_cfg.touch, touchLayout);
    failed = fprintf(f,
            "{\n"
            "    \"launcher.skip\": %s,\n"
            "    \"video.window_mode\": \"%s\",\n"
            ...
            "    \"enhancements.ambient_occlusion\": %d,\n"
            "    \"touch.enabled\": %s,\n"
            "    \"touch.size\": %d,\n"
            "    \"touch.opacity\": %d,\n"
            "    \"touch.floating\": %s,\n"
            ...
            enh.sfxVoices,
            enh.heapScale,
            enh.ambientOcclusion,
            g_cfg.touch.enabled ? "true" : "false", g_cfg.touch.size,
            g_cfg.touch.opacity, g_cfg.touch.floating ? "true" : "false", touchLayout) < 0;
    }
...
extern "C" int mp6_launcher_decide_mode(int hasNumericArg, int hasInputScript, int *outShowMenu)
{
    ...
    if (g_launcherMode) {
         ...
         * (include/mp6_enhancements.h) as soon as they are known -- before
        ...
        /* Record backend and legacy-backend AA startup selections. */
...
extern "C" double mp6_launcher_cfg_tick_hz(void)
{
    return g_launcherMode ? g_cfg.tickHz : 0.0;
}

extern "C" void mp6_msm_apply_voice_limit(void);
extern "C" int mp6_heap_scale_restart_pending(void);
extern "C" void mp6_shadow_apply_quality(void);
extern "C" void mp6_fi_savestate_reset(void);
#ifdef MP6_AURORA_LIVE_AA
extern "C" void mp6_aurora_set_aa(unsigned samples, float ssaa, int fxaa);
#endif
static bool g_settingsPending = false;
static bool g_shadowPending = false;
static int g_aaPending = -1;

/* Both event drains call this before begin_frame, never from a UI callback
 * inside a render pass. Coalesce rapid changes to the latest selection. */
extern "C" void mp6_launcher_apply_pending_settings(void)
{
    if (g_settingsPending) {
        g_settingsPending = false;
        mp6_msm_apply_voice_limit();
    }
    if (g_shadowPending) {
...
#ifdef MP6_AURORA_LIVE_AA
    if (g_aaPending >= 0) {
        const int aa = g_aaPending;
        g_aaPending = -1;
        mp6_fi_savestate_reset();
        mp6_aurora_set_aa(aa == MP6_AA_MSAA4X ? 4 : 1,
            aa == MP6_AA_SSAA15 ? 1.5f : aa == MP6_AA_SSAA2X ? 2.0f : 1.0f,
            aa == MP6_AA_FXAA);
    }
#endif
}

/* Only device selection and legacy-backend render targets need a startup
 * snapshot. Heap ownership is tracked by the allocator itself. */
...
/* Actual startup AA, including explicit environment overrides. Legacy
 * backends use these values to avoid stacking FXAA over MSAA/SSAA. */
...
/* Queue the complete live AA transaction when supported. Otherwise return
 * 1 only when the caller should update the legacy FXAA post-process. */
extern "C" int mp6_launcher_aa_apply_live(int aa)
{
    /* Presets also call this; a disabled individual row is not enough to
     * preserve explicit render overrides when a preset is selected. */
    for (const char *key : { "MP6_MSAA", "MP6_SSAA", "MP6_FXAA" }) {
        const char *value = getenv(key);
        if (value && *value) return 0;
    }
#ifdef MP6_AURORA_LIVE_AA
    g_aaPending = aa;
    return 0; /* the complete AA transaction is queued, including FXAA */
#else
    if (aa != MP6_AA_FXAA) {
        g_sessionPostAa = 0;
        return 1;
...
/* Project the resolved AA enum into Aurora's startup configuration.
 * Live changes use the frame-boundary transaction above. Widescreen,
 * interpolation and shadow consumers read the enhancement seam directly. */
extern "C" int mp6_launcher_cfg_msaa(void)
{
    return mp6_aa_to_msaa(mp6_enh_aa_mode());
}

/* Startup FXAA projection; live transitions use the AA transaction. */
...
#ifdef __ANDROID__
    /* Desktop window preferences must never exit Android immersive mode.
     * Content aspect/letterboxing is controlled independently by the renderer. */
...
extern "C" void mp6_launcher_apply_game_settings(void)
{
    ...
    /* The throttle reads config directly. Never manufacture an environment
     * override: that made the Free-Run control disable itself after boot. */
...
static const char *mp6_resource_base(void)
{
     ...
     * (gradle syncs the repo's res/ tree -- packaging/android/app/
...
void cfg_save()
{
    const int oldShadow = mp6_enh_shadow_quality();
    const int oldVoices = mp6_enh_sfx_voices();
    mp6_launcher_publish_enh();
    g_shadowPending |= oldShadow != mp6_enh_shadow_quality();
    g_settingsPending |= oldVoices != mp6_enh_sfx_voices();
    mp6_launcher_config_save();
}
...
/* Only genuinely latched resources can require a restart. Heap readiness is
 * queried without latching, so changes before Play never cause a false prompt. */
bool restart_pending()
{
    const bool heapPending = mp6_heap_scale_restart_pending() != 0;
    const bool backendPending = strcmp(g_cfg.backend, g_initialBackend) != 0;
#ifdef MP6_AURORA_LIVE_AA
    return backendPending || heapPending;
#else
    return backendPending || heapPending || mp6_aa_to_msaa(g_cfg.aa) != g_initialMsaa ||
        mp6_aa_to_ssaa(g_cfg.aa) != g_initialSsaa ||
        (g_cfg.aa == MP6_AA_FXAA ? 1 : 0) != g_sessionPostAa;
#endif
}
...
extern "C" void mp6_launcher_frame_overlay(void)
{
    ...
    if (g_console == nullptr) {
        /* THE DEV CONSOLE'S ONLY AVAILABILITY GRANT (include/mp6_console.h).
    ...
    if (mp6_minigame_take_skipped()) {
        mp6::ui::push_toast({
            .type = "minigame",
            .title = "Minigame Skipped",
            .content = "Minigames are not available yet. The board continues without minigame coin awards.",
            .duration = std::chrono::seconds(4),
        });
    }
    if (mp6_unavailable_overlay_take_returned()) {
        mp6::ui::push_toast({
            .type = "unavailable",
            .title = "Not Available Yet",
            .content = "This minigame or mode is not available yet. Returning to a supported screen.",
            .duration = std::chrono::seconds(4),
        });
    }
    mp6::ui::update();
...
{
    mp6_ao_shutdown(); /* also needed in automation, where there is no UI */
    if (!g_uiReady) return;
