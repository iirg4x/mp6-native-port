    ...
    : Document(formatted_source(kDocumentSource))
{
    ...
    if (mPopup != nullptr) {
        // Delegate to the stable popup; its rows are rebuilt on every edit.
        listen(mPopup, Rml::EventId::Click, [this](Rml::Event &event) {
            auto *row = event.GetTargetElement();
            while (row != nullptr && row != mPopup && row->GetTagName() != "acrow") {
                row = row->GetParentNode();
            }
            if (row != nullptr && row != mPopup) {
                mAcSel = row->GetAttribute<int>("match", -1);
                mPendingCompletion = true;
    ...
    listen(mDocument, Rml::EventId::Keydown, [this](Rml::Event &event) {
        ...
        // Only the printable toggle key has a following text event. F9,
        // Enter, Tab and arrows must not consume the next IME commit: Android
        // soft keyboards can send text without an intervening Keydown.
    ...
    if (auto *cmdbar = mDocument->GetElementById("cmdbar")) {
        ...
        if (mInput != nullptr) {
            ...
            /* Hidden documents can still receive a framework focus request.
             * Keep the command field inert until the console is opened. */
            mInput->SetAttribute("disabled", true);
    ...
    if (auto *cmdbar = mDocument->GetElementById("cmdbar")) {
        auto close = mDocument->CreateElement("button");
        close->SetId("console-close");
        close->SetInnerRML("Close");
        listen(close.get(), Rml::EventId::Click, [this](Rml::Event &event) {
            hide(false);
            event.StopPropagation();
        });
        cmdbar->AppendChild(std::move(close));
    }
}
...
void Console::refresh_completions(bool force)
{
    ...
    int tokenStart = 0;
    const int total = mp6_console_complete(line.c_str(), nullptr, 0, &tokenStart);
    std::vector<const char *> raw(total);
    mp6_console_complete(line.c_str(), raw.data(), total, &tokenStart);
    mAcTokenStart = tokenStart;
    mAcShown = total;
    if (lineChanged || mAcSel < 0 || mAcSel >= mAcShown) {
        mAcSel = 0;
    }
    if (mAcShown == 0) {
        hide_completions();
    ...
    mAcMatch.resize(mAcShown);
    for (int i = 0; i < mAcShown; ++i) {
        mAcMatch[i] = (raw[i] != nullptr) ? Rml::String(raw[i]) : Rml::String();
    }

    // Keep every match reachable, including "board", the ninth stat entry.
    mPopup->SetInnerRML("");
    mPopup->SetScrollTop(0);
    for (int i = 0; i < mAcShown; ++i) {
...
void Console::move_completion(int delta)
{
    ...
    for (int i = 0; i < mPopup->GetNumChildren(); ++i) {
        ...
        if (index == mAcSel) {
            child->ScrollIntoView(Rml::ScrollIntoViewOptions {
                Rml::ScrollAlignment::Center, Rml::ScrollAlignment::Center,
                Rml::ScrollBehavior::Smooth, Rml::ScrollParentage::Closest,
            });
        }
        ++index;
    }
...
{
    mp6_console_open();
    if (mInput != nullptr) mInput->RemoveAttribute("disabled");
    Document::show();
...
bool Console::focus()
{
    if (!visible() || mp6_console_is_open() == 0) return false;
...
void Console::update()
{
    ...
    if (mPendingCompletion) {
        mPendingCompletion = false;
        // Defer rebuilding rows until the touch/click event has finished.
        if (mInput != nullptr && mInput->GetValue() == mAcLine) {
            apply_completion();
            mInput->Focus(true);
        }
    }

    if (visible() && mPendingFocusFrames > 0 && --mPendingFocusFrames == 0 && mInput != nullptr) {
