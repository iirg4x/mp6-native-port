 ...
 * include/dolphin_compat.h -- keeping this file's header universe
...
#include "mp6_unlocked_fps.h" /* Mods-page Unlocked FPS: retained-stream frame interpolation
                               * (src/gx/frame_interp.c); hooks at the frame boundaries
...
#include <windows.h> /* Section 6 below (console repositioning:
                      ...
                      * windows.h exception outside src/host/: the
...
char *mp6_bridge_gpu_cache_path(const char *appName, const char *overrideRoot)
{
    /* Old Last Five Turns / Boo configurations can abort asynchronous shader
     * compilation on a later launch, before the repaired fade hook runs.
     * Preserve the legacy cache and use a new generation; userPath is unchanged.
     * Apply the same generation below a QA override so recovery tests exercise
     * the real selection policy, not a separate test-only branch. */
    const char suffix[] = "/gpu-cache-v3";
    char *defaultRoot = NULL;
    const char *root = overrideRoot;
    if (root == NULL || root[0] == '\0') {
        defaultRoot = SDL_GetPrefPath(NULL, appName);
        root = defaultRoot;
    }
    if (root == NULL) return NULL;
    size_t length = strlen(root);
    while (length > 0 && (root[length-1] == '/' || root[length-1] == '\\')) length--;
    if (length > SIZE_MAX - sizeof(suffix)) {
        SDL_free(defaultRoot);
        return NULL;
    }
    ...
    if (path != NULL) {
        ...
        if (!SDL_CreateDirectory(path)) {
            fprintf(stderr, "[BOOT] cannot create graphics cache '%s': %s\n", path, SDL_GetError());
            free(path);
            path = NULL;
        }
    }
 ...
 * (src/gx/gxarray_registry.c). decomp never has a real element count
 * on hand here, but src/hsf/hsf_load_native.c -- which allocates
 ...
 *
 * Fixed instead in include/dolphin_compat.h: every decomp call site
...
typedef struct {
    ...
    const void *data;
    u32 size;
    u8 path; /* 0=registry 1=learned 2=zero 3=explicit span */
} MP6ArrayProbeSeen;
...
static void mp6_arrayprobe_log_setarray(GXAttr attr, const void *data, u8 stride,
                                        u8 path, u32 size)
{
    static const char *pathName[4] = { "REGISTRY", "LEARNED", "ZERO", "SPAN" };
    int i;
...
static void mp6_gx_array_slots_grow_for_draw(u16 nverts)
{
    ...
    for (i = 0; i < g_gxArraySlotCount; i++) {
        ...
        /* Query the SDK's current descriptor, not the previous FIFO draw's
         * decoded state. Direct/disabled attributes must never revive an
         * unrelated array merely because this primitive has more vertices. */
        GXGetVtxDesc(slot->attr, &attrType);
        if (attrType != GX_INDEX8 && attrType != GX_INDEX16) continue;
...
void mp6_GXSetArray3(GXAttr attr, void *data, u8 stride)
{
    ...
    MP6GxArraySlot *slot = mp6_gx_array_slot_get(attr);
    /* Sprite/window arrays allocated in HuMem have a real bound too. Do not
     * infer their allocation size from a later, unrelated primitive count. */
    if (realSize == 0) realSize = mp6_heap_block_data_size(data);
    /* An authoritative bind replaces any old learned bind for this attribute.
     * Otherwise GXBegin could silently rebind the old sprite over this model. */
    ...
    if (slot) {
        /* A repeated address can be a fresh stack/static buffer lifetime.
         * Treat every explicit bind as a fresh size hint. */
        slot->data = data;
        slot->stride = stride;
        slot->registeredSize = 0;
        slot->used = true;
...
void mp6_gxarray_bind_span(int attr, const void *data, uint32_t byteSize,
                         uint8_t stride)
{
    MP6GxArraySlot *slot = mp6_gx_array_slot_get((GXAttr)attr);
    /* Display-list replay does not pass through our GXBegin size-learning
     * hook. Embedded ray vertices and strided colours therefore need a real
     * span before replay, not a zero-byte bind or an allocation-size guess.
     * Never let a later immediate draw restore the old learned pointer. */
    if (slot != NULL) slot->used = false;
    mp6_arrayprobe_log_setarray((GXAttr)attr, data, stride, 3, byteSize);
    GXSetArray((GXAttr)attr, data, byteSize, stride, /*le=*/true);
}
 ...
 * src/null/shims_manual.c) -- both read the SAME mp6_tick_count this
...
#ifdef __ANDROID__
/* On-screen touch controls (src/android/touch_pad.cpp -- an
...
static void mp6_latch_key_down_event(const SDL_Event *sdlEvent)
{
     ...
     * there, see include/mp6_console.h). */
...
/* Dev console hotkey (include/mp6_console.h). Grave/backtick is the
 ...
 * src/os/input_script.c, which is in PLATFORM_SOURCES_COMMON so the
...
static void mp6_pump_keyboard_to_pad(void)
{
...
#ifdef __ANDROID__
    {
        ...
        Mp6TouchState touch = {0};
        mp6_touch_pad_collect(&touch);
        status.button |= touch.buttons;
        if (status.stickX == 0 && status.stickY == 0) {
            status.stickX = touch.stickX;
            status.stickY = touch.stickY;
        }
        status.substickX = touch.cstickX;
        status.substickY = touch.cstickY;
        status.triggerLeft = touch.triggerL;
        status.triggerRight = touch.triggerR;
        status.analogA = (touch.buttons & PAD_BUTTON_A) ? 255 : 0;
        status.analogB = (touch.buttons & PAD_BUTTON_B) ? 255 : 0;
         ...
         * COMPLETELY SEPARATE signal from what src/gx/ui/overlay.cpp's
...
extern double mp6_launcher_cfg_tick_hz(void);
...
int mp6_tick_interpolation_possible(void)
{
    if (g_tickHz < 0.0) mp6_tick_throttle_init();
    /* Free-run returns from the throttle without opening an idle window.
     * Consult its resolved policy, including env overrides and live changes,
     * rather than duplicating the launcher's Fast Forward preference. */
    return g_tickHz > 0.0;
}
 ...
 * for how badly that goes is in this tree: include/mp6_frame_dump.h
...
static void mp6_tick_throttle_wait(void)
{
     ...
     * later in the same window. Transient queue pressure is a distinct -1
     * verdict: the worker can free a slot before this deadline, so yield and
     * retry it without decaying the CPU budget estimate. */
    fiDeclined = 0;
    for (;;) {
        int64_t remain;
        if (!mp6_tick_clock_now(&now)) {
        ...
        }
        {
            ...
            if (remainMs > MP6_TICK_SPIN_WINDOW_MS) {
                /* Unlocked FPS (include/mp6_unlocked_fps.h): spend the
                ...
                if (!fiDeclined) {
                    int replay = mp6_fi_idle_present(g_tickNextDeadline, g_tickPeriodNs);
                    if (replay != 0) {
                        if (replay < 0) mp6_host_sleep_ns(0);
                        continue;
                    }
                    fiDeclined = 1; /* final for this window -- see above */
                    /* Replay admission/build may have consumed part of the
                     * window. Never sleep using the pre-attempt remainder. */
                    continue;
                }
 ...
 *     (include/mp6_display.h holds the seam's contract; this is its
 *     one implementation.)
 *
 ...
 * with the same C-linkage-seam discipline src/gx/console/console_stats.c
...
{
    mp6_launcher_apply_pending_settings();
 ...
 * include/mp6_display.h (which console_stats.c includes in the headless
...
int mp6_display_select(const char *name)
{
    ...
    extern SDL_Window *mp6_aspect_window(void);
    SDL_Window *win = mp6_aspect_window();
    SDL_DisplayID *ids, target = 0, before, actual;
    SDL_Rect bounds;
    int count = 0, i, w, h, fullscreen;
    double bestHz = -1.0;
    if (!win || !name) return 0;
    ids = SDL_GetDisplays(&count);
    if (!ids || count == 0) { SDL_free(ids); return 0; }
    if (SDL_strcasecmp(name, "primary") == 0) {
        target = SDL_GetPrimaryDisplay();
    } else if (SDL_strcasecmp(name, "auto") == 0) {
        for (i = 0; i < count; ++i) {
            const SDL_DisplayMode *mode = SDL_GetCurrentDisplayMode(ids[i]);
            double hz = mode ? mode->refresh_rate : 0.0;
            if (hz > bestHz) { bestHz = hz; target = ids[i]; }
        }
    } else {
        i = mp6_display_pick_by_name(ids, count, name);
        if (i >= 0) target = ids[i];
    }
    SDL_free(ids);
    if (!target || !SDL_GetDisplayUsableBounds(target, &bounds)) return 0;
    before = SDL_GetDisplayForWindow(win);
    if (before == target) return 1;
    fullscreen = (SDL_GetWindowFlags(win) & SDL_WINDOW_FULLSCREEN) != 0;
    if (fullscreen && !SDL_SetWindowFullscreen(win, false)) return 0;
    SDL_SyncWindow(win);
    SDL_GetWindowSize(win, &w, &h);
    if (!SDL_SetWindowPosition(win, bounds.x + (bounds.w > w ? (bounds.w-w)/2 : 0),
                                  bounds.y + (bounds.h > h ? (bounds.h-h)/2 : 0))) {
    ...
    actual = SDL_GetDisplayForWindow(win);
    if (actual && actual != before) mp6_display_follow_crossing(before, actual, "settings");
...
/* The minigame-stub's "black screen" half (see include/mp6_boot.h).
 * src/os/dll_bridge.c's
...
static void mp6_window_metrics_invalidate(void);

static void mp6_dispatch_aurora_events(const AuroraEvent *event)
{
    /* SDL has just applied resize/DPI/orientation events. All camera and HUD
     * queries until the next event pump share these physical dimensions. */
    mp6_window_metrics_invalidate();
    ...
    while (event != NULL && event->type != AURORA_NONE) {
        ...
        if (event->type == AURORA_SDL_EVENT) {
             ...
             * include/mp6_display.h because it is SDL-typed and that
...
void VIWaitForRetrace(void)
{
    ...
    if (g_frameOpen) {
        ...
        { /* MP6_FRAME_DUMP (include/mp6_frame_dump.h): capture the frame
        ...
        { /* MP6_SHADOW_DUMP (include/mp6_shadow_dump.h): right after
    ...
    /* First width query after the event pump refreshes physical dimensions;
     * later model/HUD queries reuse them. The logical-width setter remains
     * immediate and idempotent, including on the first camera at boot. */
     ...
     * but the 3D backdrop extrude + per-scene camera setup (src/hsf/
     ...
     * empty registries) when widescreen is disabled -- see src/hsf/
    ...
    { /* freecam: sample keyboard/stick + drain event deltas ONCE per tick,
       ...
       * (include/mp6_freecam.h; inert unless the Mods toggle is on). */
 ...
 * file's header comment, case 4, and include/dolphin_compat.h's own
 ...
 * bytes for that primitive. Fixed at the source instead (compat/decomp/
 ...
 * the normal compat/decomp queue, e.g. a FaceDraw material/attribute
...
static SDL_Window *g_mp6MetricsWindow;
static int g_mp6MetricsWidth, g_mp6MetricsHeight, g_mp6MetricsValid;
...
static int mp6_window_metrics_get(int *width, int *height)
{
    if (g_mp6AspectWindow == NULL) return 0;
    if (!g_mp6MetricsValid || g_mp6MetricsWindow != g_mp6AspectWindow) {
        int w, h;
        if (!SDL_GetWindowSizeInPixels(g_mp6AspectWindow, &w, &h) || w <= 0 || h <= 0)
            return 0; /* Retry a temporarily unavailable/restoring surface. */
        g_mp6MetricsWindow = g_mp6AspectWindow;
        g_mp6MetricsWidth = w;
        g_mp6MetricsHeight = h;
        g_mp6MetricsValid = 1;
    }
    *width = g_mp6MetricsWidth;
    *height = g_mp6MetricsHeight;
    return 1;
}
...
void mp6_bridge_apply_content_aspect_policy(int aspectLockedCfg)
{
    ...
    /* mp6_widescreen_set_enabled() (src/main_native.c) must run
 ...
 * include/mp6_widescreen.h's contract.
 *
...
int mp6_widescreen_enabled(void)
{
     ...
     *
     *   2. the ENHANCEMENTS SEAM (include/mp6_enhancements.h), which
...
int mp6_widescreen_render_width(void)
{
    ...
    if (!mp6_widescreen_enabled() || !mp6_window_metrics_get(&w, &h)) {
     ...
     * include/mp6_boot.h). This IS the live device value, genuinely
 ...
 * converges: mp6_widescreen_render_width() reads the post-event-pump window
 * size every time, and mp6_widescreen_apply_render_width() (decomp-side,
 * compat/decomp/src/game/init.c.patch) is a cheap no-op whenever nothing
 ...
 * window placement, not a rendering change: once the game window exists (called from src/
