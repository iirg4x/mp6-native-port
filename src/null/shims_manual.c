...
void OSReport(const char *msg, ...)
{
     ...
     * (include/mp6_events.h). The game already narrates its own
 ...
 *   - default (aurora) build: src/gx/aurora_bridge.c provides the
 ...
 * src/gx/aurora_bridge.c for the exact same reason mp6_tick_advance
 ...
 * dance (mp6_clean_shutdown_exit, src/gx/aurora_bridge.c) even in
...
/* This tick's scripted PAD state (src/os/input_script.c), latched by
 ...
 * names are provided instead by src/gx/aurora_bridge.c (VI*) and by
...
void VIWaitForRetrace(void)
{
     ...
     * same order src/gx/aurora_bridge.c uses (refresh the virtual pad
 ...
 * src/main_native.c's non-headless branch) -- these four always
 * report "disabled, native size", so compat/decomp/src/game/init.c.patch's
 ...
 * no longer stubs any CARD symbol), configured by src/os/
 ...
 * src/gx/aurora_bridge.c as a defer-to-VIWaitForRetrace queue; THIS
