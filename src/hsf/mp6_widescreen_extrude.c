 ...
 * include/mp6_widescreen.h for the public contract; this is the real
 ...
 * src/hsf/ (alongside hsf_load_native.c, the real HSF deserializer
 ...
 * src/gx/aurora_bridge.c's VIWaitForRetrace, so a continuous
 ...
 * port's `MP6_LoadHSFNative`, src/hsf/hsf_load_native.c) fresh, which
...
typedef enum {
    ...
    MP6_WS_EXTEND_RESULTS /* four independently transformed theater borders */
} MP6WSExtendMode;
...
static void mp6_ws_extend_results_apply(MP6WSExtendEntry *e, float k)
{
    /* Both mirrored halves point outward along their LOCAL +X. Their
     * original transforms (including the reveal motion) place the strips.
     * Extend by the stage's world half-width, not the small mesh's width. */
    float dx = 800.0f * (k > 1.0f ? k - 1.0f : 0.0f);
    e->obj->mesh.mesh.min = e->nativeMin;
    e->obj->mesh.mesh.max = e->nativeMax;
    for (s16 i = 0; i < e->rimCount; i++) {
        HuVecF *v = e->vtx + e->baseVtx + i;
        v->x = dx == 0.0f ? e->rimX[i] : e->rimX[i] + dx;
        v->y = e->rimY[i];
        v->z = e->rimZ[i];
        if (v->x > e->obj->mesh.mesh.max.x) e->obj->mesh.mesh.max.x = v->x;
    }
}
...
/* Results DATA_mdpresult[0] packs the floor, props and proscenium into one
 * asset. Only these four border meshes continue sideways. Like Party's
 * pillar, native vertices/UVs stay untouched; appended quads pin each
 * source edge's own UVs/normals and inherit its material and winding.
 * Unlike Party's one symmetric mesh, each results half has a local +X
 * outer edge and the left half is rotated 180 degrees by the asset.
 * The measured cut is world X = +/-800. No floor/prop duplication. */
typedef struct {
    const char *name;
    s32 vertexCount, edgeCount, quadCount;
    float edgeX, posX, rotY;
} MP6WSResultsBorder;

static void mp6_ws_extend_write_faces_results(MP6WSExtendEntry *e, HSF_FACE *tail,
                                             const HSF_FACE *nativeFaces)
{
    for (s32 q = 0; q < e->rimCount / 2; q++) {
        HSF_FACE *f = &tail[q];
        const HSF_FACE *src = &nativeFaces[g_mp6WsPillarScratch.srcFace[q]];
        *f = *src;
        f->typeSrc = HSF_FACE_QUAD;
        for (s16 c = 0; c < 4; c++) {
            f->index[c].vertex = g_mp6WsPillarScratch.vertex[q][c];
            f->index[c].st = g_mp6WsPillarScratch.st[q][c];
            f->index[c].normal = g_mp6WsPillarScratch.normal[q][c];
...
static BOOL mp6_ws_results_border_edges(HSF_OBJECT *o, const MP6WSResultsBorder *profile,
                                        MP6WSExtendEntry *e)
{
    HuVecF *v = (HuVecF *)o->mesh.vertex->data;
    HSF_FACE *faces = (HSF_FACE *)o->mesh.face->data;
    HSF_TRANSFORM *t = &o->mesh.base;
    s32 edgeCount = 0, q = 0;
    if (o->mesh.vertex->count != profile->vertexCount
        || o->mesh.face->count > 32767
        || (o->mesh.color && o->mesh.color->count > 0)
        || !mp6_ws_pillar_near(t->pos.x, profile->posX, 0.02f)
        || !mp6_ws_pillar_near(t->rot.y, profile->rotY, 0.001f)
        || t->rot.x != 0.0f || t->rot.z != 0.0f
        || t->scale.x != 1.0f || t->scale.y != 1.0f || t->scale.z != 1.0f) return FALSE;
    for (s32 i = 0; i < o->mesh.vertex->count; i++) {
        if (v[i].x > profile->edgeX + 0.02f) return FALSE;
        if (mp6_ws_pillar_near(v[i].x, profile->edgeX, 0.02f)) edgeCount++;
    }
    if (edgeCount != profile->edgeCount) return FALSE;
    for (s32 fi = 0; fi < o->mesh.face->count; fi++) {
        HSF_FACE *f = &faces[fi];
        s16 n = mp6_ws_pillar_face_corners(f);
        if (!n) {
            if ((f->typeSrc & HSF_FACE_MASK) != HSF_FACE_TRISTRIP) return FALSE;
            /* The measured outer edges are TRI/QUAD boundaries. Never
             * silently omit an edge if a future asset uses a strip there. */
            for (s32 c = 0; c < 3 + (s32)f->strip.count; c++) {
                HSF_FACE_INDEX *p = c < 3 ? &f->strip.index[c] : &f->strip.data[c - 3];
                if (p->vertex < 0 || p->vertex >= o->mesh.vertex->count
                    || mp6_ws_pillar_near(v[p->vertex].x, profile->edgeX, 0.02f)) return FALSE;
            }
            continue;
        }
        for (s16 c = 0; c < n; c++) {
            HSF_FACE_INDEX *p = &f->index[c];
            if (p->vertex < 0 || p->vertex >= o->mesh.vertex->count
                || p->st < 0 || p->st >= o->mesh.st->count
                || p->normal < 0 || p->normal >= o->mesh.normal->count) return FALSE;
        }
        for (s16 c = 0; c < n; c++) {
            HSF_FACE_INDEX a = f->index[mp6_ws_pillar_emission_slot(n, c)];
            HSF_FACE_INDEX b = f->index[mp6_ws_pillar_emission_slot(n, (c + 1) % n)];
            if (!mp6_ws_pillar_near(v[a.vertex].x, profile->edgeX, 0.02f)
                || !mp6_ws_pillar_near(v[b.vertex].x, profile->edgeX, 0.02f)) continue;
            if (q >= profile->quadCount || q >= MP6_WS_EXTEND_RIM_MAX / 2) return FALSE;
            /* A shared/internal edge must not be extruded twice. */
            for (s32 j = 0; j < q; j++) {
                s16 va = e->rimVtx[2*j], vb = e->rimVtx[2*j+1];
                if ((va == a.vertex && vb == b.vertex) || (va == b.vertex && vb == a.vertex)) return FALSE;
            }
            s16 da = (s16)(e->baseVtx + 2*q), db = da + 1;
            /* Native cycle a->b: continuation runs b->a->da->db. */
            s16 cv[4] = {b.vertex, a.vertex, da, db};
            s16 cs[4] = {b.st, a.st, a.st, b.st};
            s16 cn[4] = {b.normal, a.normal, a.normal, b.normal};
            mp6_ws_pillar_stage_quad(q, fi, cv, cs, cn);
            for (s16 j = 0; j < 2; j++) {
                s16 vi = j ? b.vertex : a.vertex, d = (s16)(2*q+j);
                e->rimVtx[d] = vi;
                e->rimX[d] = v[vi].x;
                e->rimY[d] = v[vi].y;
                e->rimZ[d] = v[vi].z;
    ...
    e->rimCount = (s16)(2*q);
    return q == profile->quadCount;
}

void mp6_widescreen_extend_results_set(int16_t modelId)
{
    static const MP6WSResultsBorder borders[] = {
        {"obj28",       67, 5, 4, 237.645f, -562.355f, 180.0f},
        {"obj29",       67, 5, 4, 237.645f,  562.355f,   0.0f},
        {"stage_base",  68, 3, 2, 262.671f,  537.329f,   0.0f},
        {"stage_base2", 68, 3, 2, 262.671f, -537.329f, 180.0f},
    };
    for (s32 i = 0; i < 4; i++) {
        const MP6WSResultsBorder *p = &borders[i];
        HSF_OBJECT *o = mp6_ws_extend_locate(modelId, p->name);
        if (o == NULL) continue;
        MP6WSExtendEntry *e = mp6_ws_extend_find(o);
        if (e == NULL) {
            MP6WSExtendEntry pending = {0};
            pending.modelId = modelId;
            pending.hsf = Hu3DData[modelId].hsf;
            pending.obj = o;
            pending.mode = MP6_WS_EXTEND_RESULTS;
            pending.nativeMin = o->mesh.mesh.min;
            pending.nativeMax = o->mesh.mesh.max;
            pending.baseVtx = o->mesh.vertex->count;
            pending.baseSt = o->mesh.st->count;
            if (!mp6_ws_results_border_edges(o, p, &pending)) {
                mp6_ws_extend_refuse(p->name, "not the measured results border layout");
                continue;
            }
            e = mp6_ws_extend_alloc();
            if (e == NULL) continue;
            *e = pending;
            if (!mp6_ws_extend_grow(modelId, o, e->rimCount, 0, p->quadCount,
                                    mp6_ws_extend_write_faces_results, e)) {
                e->obj = NULL;
                e->hsf = NULL;
                continue;
            }
            ...
            e->st = (HuVec2f *)o->mesh.st->data;
            if (getenv("MP6_WS_HSF_DIAG"))
                printf("[MP6-WS-EXTEND] results %s: appended %d pinned-UV quads\n", p->name, p->quadCount);
        }
        mp6_ws_extend_apply(e, mp6_widescreen_scale_factor());
    }
    GXInvalidateVtxCache();
}
 ...
 * src/gx/aurora_bridge.c's VIWaitForRetrace (the same site the
...
void mp6_widescreen_debug_dump_model(HU3D_MODELID modelId, const char *label)
{
    ...
    for (i = 0; i < hsf->objectNum; i++) {
         ...
         * by a per-object `vertexId` -- src/hsf/hsf_load_native.c's
