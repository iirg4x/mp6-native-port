/* MP6 native port -- the src/host/ seam.
 *
...
#ifdef _WIN32
/* GUI releases have no console. Preserve explicitly redirected output (tests
 * and command-line diagnostics); otherwise log next to the working saves,
 * keeping the preceding session for crash investigation. */
 ...
 * include/mp6_heap_scale.h), which silently relocated the coroutine
 ...
 * own policy decision (include/mp6_savestate.h). */
int mp6_host_image_writable_sections(Mp6HostImageSection *out, int maxOut);

/* Coroutine-pool introspection for savestates (src/host/coro_arena.c).
