 ...
 * See include/mp6_savestate.h for the design rationale and the
 * carve-out contract.
 *
 * FILE LAYOUT
...
/* Arena accessors (src/os/arena.c). Declared here rather than in a
...
/* The fake-ARAM buffer (src/null/shims_manual.c). Declared here rather
...
int mp6_savestate_capture(const char *path)
{
     ...
     * src/dvd/dvd_files.c's own comment). Enabling THP or compiling
...
int mp6_savestate_restore(const char *path)
{
     ...
     * reservation that has to hold them (src/os/arena.c,
     * include/mp6_heap_scale.h), so the same exe reserves 256 MB at x1
     ...
     * scale latch (src/os/heap_scale.c), never a captured static, so
     ...
     * image's .bss early on, and g_fakeAram (src/null/shims_manual.c)
    ...
    /* Dev console sampler (include/mp6_console.h). Both console TUs are
...
/* The tick counter both build modes advance (src/null/shims_manual.c),
