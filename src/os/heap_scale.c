 ...
 * latch. The arithmetic lives in include/mp6_heap_scale.h (pure, and
 ...
 * moments that are thousands of instructions apart: src/os/arena.c
 * reserves the game arena during mp6_arena_init(), and
 * src/os/malloc_direct.c sizes the four fixed HuMem heaps inside
...
{
    return g_scale != 0u && g_scale != mp6_heap_scale_clamp((long)mp6_enh_heap_scale());
}
