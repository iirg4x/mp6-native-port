...
`node packaging/web/test/run.js` covers the FST parser, the wanted-set filter, CRC-32,
...
`python -m http.server` from `packaging/web/`) and confirmed: the whole ES module
...
real disc): everything below. Serve `packaging/web/` locally
(`python -m http.server 8000` from inside `packaging/web/`, then open
...
- [ ] `.github/workflows/pages.yml` actually deploys `packaging/web/` once Pages is
