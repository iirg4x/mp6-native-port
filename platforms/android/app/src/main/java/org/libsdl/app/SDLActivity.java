        ...
        if (mNextNativeState == NativeState.RESUMED) {
            if (mSurface.mIsSurfaceReady && (mHasFocus || mHasMultiWindow) && mIsResumedCalled) {
                if (mSDLThread == null) {
                    ...
                    // The ENTIRE game runs on this thread (nativeRunMain ->
                    // mp6_android_main -> GameMain), compiled -O0 like every
                    // decomp TU, and the region is shared with ART's Java
                    // frames for every JNI upcall SDL makes -- the default
                    // (~4MB per the device's StackOverflowError message) is
                    // thin for -O0 game code + Vulkan driver + ART headroom.
                    // NOTE, recorded honestly: this is HEADROOM, not the fix
                    // for the U-A3 fileselect crash -- that abort reproduced
                    // identically at 32MB ("stack size 33MB") and was root-
                    // caused to JNI upcalls from game coroutine stacks (see
                    // mp6_PADControlMotor in platform/gx/aurora_bridge.c and
                    // aurora patch 0011). 32MB is address space, not memory
                    // (commit-on-touch).
                    mSDLThread = new Thread(null, new SDLMain(), "SDLThread", 32 * 1024 * 1024);
