 ...
 * indexes into (GX_INDEX16 addressing). src/gx/aurora_bridge.c's
 ...
 * src/hsf/hsf_load_native.c (which allocates every vertex/normal/st/
 ...
 * src/hsf/hsf_load_native.c (compiled in both build modes, decomp
 * header universe) and src/gx/aurora_bridge.c (Aurora-only, Aurora's
...
/* Immediate, exact-byte binding for arrays embedded in particle allocations.
 * No persistent registration: each draw owns the span and rebinds it. This
 * also retires the attribute's previous learned-size hint. */
void mp6_gxarray_bind_span(int attr, const void *data, uint32_t byteSize,
                         uint8_t stride);
/* Allocator lifetime seams; bulk invalidation runs before metadata is freed.
 * The table remains ordinary game state, so quick-state restore rewinds it. */
