 ...
 *     include/selmenu_compat.h (-include'd for that ONE file only;
 *     doing it here in the GLOBAL compat header instead broke
 *     src/null/shims_generated.c, which -- living outside
 ...
 * resolved names). Hook point so src/gx/aurora_bridge.c can track
 ...
 * hook point so src/gx/aurora_bridge.c can count/optionally-hide
 ...
 * src/gx/aurora_bridge.c can DEFER the motor command to the next
...
/* Memory card slot A (src/os/card_native.c): the SDK's CARDInit is
...
/* FRAMESCOPE (fable diagnostic toolkit, src/gx/framescope.c): reroute
 ...
 * see src/gx/aurora_bridge.c's own comment). Two same-named, ABI-
 ...
 * mp6_GXSetArray3 -- src/gx/aurora_bridge.c (aurora build) and
 ...
 * src/gx/aurora_bridge.c (NOT an arity bridge like mp6_GXSetArray3
 ...
 * src/gx/aurora_bridge.c can track open/close balance and log
