 ...
 * src/main_native.c (owns it, parses the CLI arg) and
 * src/null/shims_manual.c (VIWaitForRetrace counts against it). */
#ifndef MP6_BOOT_H
#define MP6_BOOT_H
 ...
 * src/main_native.c's Aurora (non-headless) main() sets this when
 ...
 * (src/null/shims_manual.c's own VIWaitForRetrace) -- see
 ...
 * src/gx/aurora_bridge.c's VIWaitForRetrace calls this instead of
 ...
 * -- and this port's own aurora log callback (src/main_native.c)
 ...
 * declared in src/host/host.h -- call-once-early contract. */
 ...
 * src/gx/aurora_bridge.c's GetConsoleProcessList guard that protects
 ...
 * src/gx/aurora_bridge.c (Aurora/non-headless build only -- headless
 * has no window or console-overlap concern at all). */
void mp6_bridge_post_window_init(void *sdlWindow);
/* Select/create a versioned graphics-cache directory without moving user data.
 * Returns a malloc-owned path; caller frees it after Aurora copies the config.
 * NULL is an error, not permission to fall back to the legacy cache. */
 ...
 *       exists but sits off-screen -- see src/main_native.c's
 ...
 * starts). Defined in src/gx/aurora_bridge.c (Aurora/non-headless
 * build only). */
void mp6_bridge_apply_content_aspect_policy(int aspectLockedCfg);

/* Temporary black-frame cover while an unavailable overlay returns through
 * the real overlay manager. Never a permanent placeholder. Normal board
 * minigames are skipped before any transition by minigame_stub.c. */
 ...
 * (src/host/host_win32.c dbghelp; host_android.c dladdr). */
void mp6_symbolize_addr(void *addr, char *outBuf, size_t outBufSz);

/* src/os/malloc_direct.c's all-heap allocation census -- see that
 * file's own header comment above HuMemDirectMalloc for the full design.
 * Called once per tick from mp6_tick_advance() (src/null/
...
/* src/hsf/mp6_motion_leaktest.c's standalone motion-bank ownership
 ...
 * over-reports. Defined in src/os/malloc_direct.c next to the
