 ...
 * defaults (src/gx/ui/launcher_core.cpp): a first interactive run with
 ...
 * compiles src/enh/mp6_enhancements.c and drives them directly.
 */
 ...
 * Accessors always answer current preferences. Render-resource changes are
 * queued at a frame boundary; voice-table changes take the mixer lock.
 * Only allocated game heaps stay fixed until restart. Older Aurora builds
 * also retain their startup MSAA/SSAA targets.
 * --------------------------------------------------------------------- */
...
int mp6_enh_aa_mode(void);         /* Mp6EnhAaMode; live with MP6_AURORA_LIVE_AA. */
int mp6_enh_ambient_occlusion(void); /* 0 Off, 1 Subtle, 2 Strong. LIVE; opt-in. */

/* 16 (retail) or 32. Applied under the mixer lock. Shrinking retires voices
 * in removed slots; surviving voice handles and slot indices do not move. */
int mp6_enh_sfx_voices(void);

/* 1 (retail) or 4. LATCHED process-wide by src/os/heap_scale.c on first
...
/* Mirrors Mp6AaMode (src/gx/ui/launcher_state.hpp), which is the C++
...
typedef struct Mp6EnhValues {
    ...
    int sfxVoices;     /* 16 / 32 */
    int heapScale;     /* 1 / 4 */
    int ambientOcclusion; /* 0 / 1 / 2; all presets leave it off */
} Mp6EnhValues;
...
/* Resolve startup environment overrides once after host arguments are parsed.
 * Hot drawing paths then read the resolved store without getenv/strtol work.
 * set_values still publishes live changes immediately, including overrides.
 * Call again only if a diagnostic changes the process environment at runtime.
 * Before this opt-in call, accessors retain their uncached bootstrap behavior. */
